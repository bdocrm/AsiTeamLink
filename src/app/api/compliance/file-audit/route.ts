...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[file-audit] Supabase query error:', JSON.stringify(error));
      return NextResponse.json(
        { error: 'Failed to fetch file audit logs', detail: error.message },
        { status: 500 }
      );
    }
    ...
    console.error('[file-audit] Unhandled error:', err?.message ?? err);
    return NextResponse.json(
